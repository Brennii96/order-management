...
// Could be some issues with this in future...
$orderId = DB::getInstance()->query("SELECT orders_id FROM orders where `status` = '" . Session::get('status') . "' AND `payment_method` = '" . Session::get('payment_method') . "' AND `date_time` = '" . Session::get('date_time') ."' AND `description` = '" . Session::get('description') ."';");
if (Input::exists()) {
        ...
        if ($validation->passed()) {
            try {
                $productsToOrder->create(array(
                    'orders_id' => $orderId,
                    'clients_id' => (int)Input::get('client'),
...
<head>
    ...
    <title>Attach Client and Add Products | Order Management</title>
</head>
<body>
